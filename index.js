...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Vincent\n' // Seu nome
            + 'ORG:Lexa;\n' // Nome do Bot
            + 'TEL;type=CELL;type=VOICE;waid=5589981210976:+55 67 99334-1643\n' // Numero do Whatsapp
            + 'END:VCARD'
prefix = '*','#','+','!'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '[❗] Meliodas? Este é um recurso especial para o Vincent ❌',
					ownerB: '[❗] Vincent? Este é um recurso especial para o Vincent ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmão. Você não está registrado como amigo do Vincent. Registre-se para fazer amizade com o bot ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎ por meio, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎|17\n\n──「 BOT MAN 」──`,
			...
			const ownerNumber = ["556793341643@s.whatsapp.net"] // Recoloque o seu numero
			const mod = [ownerNumber,"556793341643@s.whatsapp.net"]// não sei o que eu isso 
			const adminbotnumber = ["556793341643@s.whatsapp.net"]// admin bot numero
			const frendsowner = ["556793341643@s.whatsapp.net"]// amigo do criador 
			const premium = ["556793341643@s.whatsapp.net"]
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("*vincent")){
			...
			if (messagesC.includes("@556793341643")){
			client.updatePresence(from, Presence.composing)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎ BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎ domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					case 'addvip':  
					if (!isOwner) return reply(mess.only.ownerB)
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎ ou digite * # Daftarvip * para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎ ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎* 」*────*\n\n*_SE QUER SER ADMIN DO BOT ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/554792091566/*╰──────*「 * ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎ BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do ✞︎𝚅𝚒𝚗𝚌𝚎𝚗𝚝✞︎:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
